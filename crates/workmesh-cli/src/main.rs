...
enum Command {
    ...
    /// Workstream orchestration for parallel feature streams
    Workstream {
        #[command(subcommand)]
...
enum ServiceCommand {
    ...
    /// Install/update a systemd unit for workmesh-service
    InstallSystemd {
        #[arg(long)]
        config: Option<PathBuf>,
        #[arg(long)]
        ...
        #[arg(long)]
        request_timeout_ms: Option<u64>,
        /// Installation scope (`user` writes to ~/.config/systemd/user, `system` writes to /etc/systemd/system)
        #[arg(long, value_enum, default_value_t = ServiceInstallScopeArg::User)]
        scope: ServiceInstallScopeArg,
        /// Unit name (with or without .service suffix)
        #[arg(long, default_value = "workmesh-service")]
        unit_name: String,
        /// Run `systemctl enable` after installing/updating the unit
        #[arg(long, action = ArgAction::SetTrue)]
        enable: bool,
        /// Run `systemctl start` after installing/updating the unit
        #[arg(long, action = ArgAction::SetTrue)]
        start: bool,
        /// Preview actions without writing files or running systemctl
        #[arg(long, action = ArgAction::SetTrue)]
        dry_run: bool,
        /// Print generated unit file content
        #[arg(long, action = ArgAction::SetTrue)]
        print_unit: bool,
        #[arg(long, action = ArgAction::SetTrue)]
        json: bool,
...
enum WorkstreamCommand {
    ...
    /// Pause a workstream (intentionally inactive)
    Pause {
        /// Workstream id or key (defaults to active workstream)
        id: Option<String>,
        #[arg(long, action = ArgAction::SetTrue)]
        json: bool,
    },
    /// Close a workstream (completed or abandoned)
    Close {
        /// Workstream id or key (defaults to active workstream)
        id: Option<String>,
        #[arg(long, action = ArgAction::SetTrue)]
    ...
    /// Reopen a paused/closed workstream (marks it active)
    Reopen {
        /// Workstream id or key (defaults to active workstream)
        id: Option<String>,
        #[arg(long, action = ArgAction::SetTrue)]
    ...
    /// Update workstream fields (key, notes, context snapshot)
    Set {
        /// Workstream id or key (defaults to active workstream)
        id: Option<String>,
        #[arg(long)]
        ...
        #[arg(long)]
        objective: Option<String>,
        /// Optional task list (CSV) to set scope.mode=tasks (ignored if --epic is set)
        #[arg(long)]
        tasks: Option<String>,
        #[arg(long, action = ArgAction::SetTrue)]
        json: bool,
    },
    /// Switch active workstream for this worktree
    Switch {
        id: String,
        #[arg(long, action = ArgAction::SetTrue)]
        json: bool,
    },
    /// Diagnose workstream registry health for this repo
    Doctor {
        #[arg(long, action = ArgAction::SetTrue)]
        json: bool,
    },
...
enum WorktreeCommand {
    ...
    /// Convert a standalone clone into a git worktree under this repo (backup + worktree add)
    AdoptClone {
...
enum TruthCommand {
    ...
    Propose {
        ...
        #[arg(long)]
        feature: Option<String>,
        /// Optional workstream id (defaults to the active workstream when --current is set)
        #[arg(long)]
        ...
        /// Auto-fill workstream/session/worktree fields from the current working directory and context.
        #[arg(long, action = ArgAction::SetTrue)]
        current: bool,
        #[arg(long)]
        actor: Option<String>,
...
fn handle_service_command(root: &Path, command: &ServiceCommand) -> Result<()> {
    match command {
        ServiceCommand::Start {
            ...
            let service_binary = resolve_workmesh_service_binary()?;
            let mut cmd = std::process::Command::new(&service_binary);
            ...
            let status = cmd.status().map_err(|err| {
                anyhow!("failed to execute {}: {}", service_binary.display(), err)
            })?;
            if !status.success() {
                return Err(anyhow!(
                    "{} exited with status {}",
        ...
        ServiceCommand::Verify { json } => {
            let service_binary = resolve_workmesh_service_binary()?;
            let output = std::process::Command::new(&service_binary)
                .arg("--version")
                .output()
                .map_err(|err| {
                    anyhow!("failed to execute {}: {}", service_binary.display(), err)
                ...
                if *json {
                    println!(
                        ...
                        serde_json::to_string_pretty(&serde_json::json!({
                            "ok": true,
                            "binary": service_binary.display().to_string(),
                            "version": stdout,
                        }))?
                    );
                } else {
                    println!("{}: {}", service_binary.display(), stdout);
                }
                Ok(())
            } else {
                if *json {
        ...
        ServiceCommand::InstallSystemd {
            ...
            let root_dir = root
                ...
                .to_string_lossy()
                .to_string();
            let service_bin = resolve_workmesh_service_binary()?;
            let unit_dir = match scope {
                ServiceInstallScopeArg::User => {
                    let home =
                        std::env::var("HOME").context("HOME is required for --scope user")?;
                    PathBuf::from(home).join(".config/systemd/user")
                }
                ServiceInstallScopeArg::System => PathBuf::from("/etc/systemd/system"),
            };
            ...
            if let Some(config) = config {
                ...
                service_args.push(config.to_string_lossy().to_string());
            }
            if let Some(host) = host {
                service_args.push("--host".to_string());
                service_args.push(host.clone());
            }
            if let Some(port) = port {
                service_args.push("--port".to_string());
                service_args.push(port.to_string());
            }
            if let Some(log_filter) = log_filter {
                service_args.push("--log-filter".to_string());
                service_args.push(log_filter.clone());
            }
            if let Some(auth_token) = auth_token {
                service_args.push("--auth-token".to_string());
                service_args.push(auth_token.clone());
            }
            if let Some(max_body_bytes) = max_body_bytes {
                service_args.push("--max-body-bytes".to_string());
                service_args.push(max_body_bytes.to_string());
            }
            if let Some(request_timeout_ms) = request_timeout_ms {
                service_args.push("--request-timeout-ms".to_string());
                service_args.push(request_timeout_ms.to_string());
            }

            let exec_start = format!(
                "{}{}",
                escape_systemd_arg(service_bin.to_string_lossy().as_ref()),
                if service_args.is_empty() {
                    ...
                    format!(
                        ...
                        service_args
                            .iter()
                            .map(|arg| escape_systemd_arg(arg))
                            .collect::<Vec<_>>()
                            .join(" ")
                    )
                }
            ...
            let wanted_by = match scope {
                ServiceInstallScopeArg::User => "default.target",
                ServiceInstallScopeArg::System => "multi-user.target",
            };
            let unit_body = format!(
                "[Unit]\nDescription=WorkMesh Service\nAfter=network-online.target\nWants=network-online.target\n\n[Service]\nType=simple\nWorkingDirectory={}\nExecStart={}\nRestart=on-failure\nRestartSec=2\n\n[Install]\nWantedBy={}\n",
                escape_systemd_arg(&root_dir),
                exec_start,
                wanted_by
            );
            ...
            if *dry_run {
                if *json {
                    ...
                    println!(
                        "Dry run: would install {} at {}",
                        unit_name,
                        unit_path.display()
                    );
                    println!("WorkingDirectory={}", root_dir);
                    println!("ExecStart={}", exec_start);
                    println!("Enable after install: {}", enable);
                    println!("Start after install: {}", start);
                    println!();
                    println!("{}", unit_body);
                }
                return Ok(());
            ...
            if *enable && *start {
                run_systemctl(*scope, &["enable", "--now", &unit_name])?;
            } else {
                if *enable {
            ...
            if *json {
                println!(
                    ...
                    serde_json::to_string_pretty(&serde_json::json!({
                        ...
                        "enable_requested": enable,
                        "start_requested": start,
                        "printed_unit": print_unit,
                        "unit": if *print_unit { serde_json::Value::String(unit_body.clone()) } else { serde_json::Value::Null },
                        "status_hint": systemctl_status_hint(*scope, &unit_name),
                    }))?
                );
            } else {
                println!("Installed {} at {}", unit_name, unit_path.display());
                ...
                println!("Status: {}", systemctl_status_hint(*scope, &unit_name));
                if !*enable && !*start {
                    println!(
                        "Next: {}",
                        systemctl_cmd_line(*scope, &["enable", "--now", &unit_name])
                    );
                }
            }
...
fn normalize_systemd_unit_name(name: &str) -> String {
...
fn resolve_workmesh_service_binary() -> Result<PathBuf> {
    if let Ok(path) = which::which("workmesh-service") {
        return Ok(path);
    }

    if let Ok(current_exe) = std::env::current_exe() {
        if let Some(parent) = current_exe.parent() {
            let candidate = parent.join("workmesh-service");
    ...
    Err(anyhow!(
        "workmesh-service not found (install it in PATH or build alongside workmesh)"
    ))
}

fn escape_systemd_arg(raw: &str) -> String {
...
fn systemctl_cmd_line(scope: ServiceInstallScopeArg, args: &[&str]) -> String {
    let mut parts = vec!["systemctl".to_string()];
    if scope == ServiceInstallScopeArg::User {
...
fn systemctl_status_hint(scope: ServiceInstallScopeArg, unit_name: &str) -> String {
    systemctl_cmd_line(scope, &["status", unit_name])
}

fn run_systemctl(scope: ServiceInstallScopeArg, args: &[&str]) -> Result<()> {
    ...
    let output = cmd
        .output()
        .with_context(|| format!("failed to execute {}", systemctl_cmd_line(scope, args)))?;
    if output.status.success() {
        return Ok(());
    }
    let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
...
fn main() -> Result<()> {
    ...
    if let Command::Service { command } = &cli.command {
        handle_service_command(&cli.root, command)?;
        return Ok(());
    }
    ...
    match cli.command {
        ...
        Command::Truth { command } => match command {
            TruthCommand::Propose {
                ...
                let resolved_workstream_id = workstream_id.or(inferred_workstream_id.clone());
                let resolved_feature = if feature.is_some() {
                    feature
                } else if current {
                    resolved_workstream_id
                        .clone()
                        .or_else(|| resolved_epic.clone())
                } else {
                    resolved_epic
                        .clone()
                        .or_else(|| resolved_workstream_id.clone())
                };
        ...
        Command::Workstream { command } => {
            let repo_root = repo_root_from_backlog(&backlog_dir);
            let home = resolve_workmesh_home()?;
            handle_workstream_command(&backlog_dir, &repo_root, &home, command)?;
        }
...
fn resolve_workstream_for_repo(
    ...
    if trimmed.is_empty() {
        return Err(anyhow::anyhow!("workstream id/key is blank"));
    }

    let streams = list_workstreams_for_repo(home, repo_root)?;
    if streams.is_empty() {
        return Err(anyhow::anyhow!("no workstreams found for this repo"));
    }
    ...
    Err(anyhow::anyhow!("workstream not found: {}", trimmed))
}

fn handle_workstream_command(
    backlog_dir: &Path,
    repo_root: &Path,
    home: &Path,
    command: WorkstreamCommand,
    ...
    match command {
        ...
        WorkstreamCommand::Pause { id, json } => {
            let active_id = load_context(backlog_dir)?
                .and_then(|ctx| ctx.workstream_id)
                .filter(|value| !value.trim().is_empty());
            let query = id
            ...
            let record = resolve_workstream_for_repo(home, &registry_repo_root, &query)?;
            let updated = update_workstream_for_repo_by_id(
                home,
                &registry_repo_root,
                &record.id,
                |record| {
            ...
            let cleared_active = active_id
                .as_deref()
                .map(|value| value == updated.id)
                .unwrap_or(false);
            if cleared_active {
            ...
            if json {
                ...
                println!("Paused workstream {}", updated.id);
                if cleared_active {
                    println!("Cleared active workstream pointer in this worktree.");
                }
            }
        }
        WorkstreamCommand::Close { id, json } => {
            let active_id = load_context(backlog_dir)?
                .and_then(|ctx| ctx.workstream_id)
                .filter(|value| !value.trim().is_empty());
            let query = id
            ...
            if json {
                ...
                println!("Closed workstream {}", updated.id);
                if cleared_active {
                    println!("Cleared active workstream pointer in this worktree.");
                }
            }
        }
        WorkstreamCommand::Reopen { id, json } => {
            let active_id = load_context(backlog_dir)?
                .and_then(|ctx| ctx.workstream_id)
                .filter(|value| !value.trim().is_empty());
            let query = id
            ...
            if json {
                ...
                println!("Reopened workstream {}", updated.id);
            }
        }
        WorkstreamCommand::Rename { id, name, json } => {
            let active_id = load_context(backlog_dir)?
                .and_then(|ctx| ctx.workstream_id)
                .filter(|value| !value.trim().is_empty());
            let query = id
            ...
            if json {
                ...
                println!("Renamed workstream {} -> {}", updated.id, updated.name);
            }
        }
        WorkstreamCommand::Set {
            id,
            key,
            notes,
            project,
            ...
            let desired_key = key.as_deref().map(|value| value.trim().to_string());
            ...
            let task_ids = tasks
                ...
                .map(split_csv)
                .unwrap_or_default();
            if let Some(epic_id) = epic.as_deref() {
            ...
            let context_changed =
                project.is_some() || objective.is_some() || epic.is_some() || tasks.is_some();
            if context_changed {
                ...
                if let Some(obj) = objective
                    ...
                    .filter(|v| !v.is_empty())
                {
                    snapshot.objective = Some(obj.to_string());
                }
            ...
            // If we're updating the active workstream in this checkout, keep context.json in sync.
            ...
            if json {
                ...
                println!("Updated workstream {}", updated.id);
                if should_update_context {
        ...
        WorkstreamCommand::Create {
            ...
            let task_ids = tasks
                ...
                .map(split_csv)
                .unwrap_or_default();
            let branch_hint = branch
                .clone()
                .or_else(|| path.as_ref().and_then(|path| current_worktree_branch(path)));
            ...
            let (created_worktree, binding, context_seeded, warnings) = if existing {
                let selected_path = path.as_ref().expect("path required for --existing");
                ...
                (None, binding, seeded, warnings)
            } else if let (Some(path), Some(branch)) = (path.as_ref(), branch.as_ref()) {
            ...
            if json {
                ...
                println!("Created workstream {}", inserted.id);
                if let Some(worktree) = inserted.worktree.as_ref() {
                    println!("worktree: {}", worktree.path);
                }
                if let Some(created) = created_worktree.as_ref() {
                    println!("Created git worktree at {}", created.path);
                }
                for warning in warnings {
                    println!("warning: {}", warning);
                }
        ...
        WorkstreamCommand::Switch { id, json } => {
            ...
            if json {
                ...
                println!("Switched active workstream to {}", record.id);
    ...
    match command {
        ...
        WorktreeCommand::AdoptClone {
            ...
            if json {
                ...
                println!("repo_root: {}", outcome.plan.repo_root);
                println!("from: {}", outcome.plan.from_path);
                println!("to: {}", outcome.plan.to_path);
                println!("source_branch: {}", outcome.plan.source_branch);
                println!("target_branch: {}", outcome.plan.target_branch);
                println!("dirty: {}", outcome.plan.dirty);
                ...
                if outcome.applied {
                    ...
                    if let Some(worktree) = outcome.worktree.as_ref() {
                        println!("worktree.path: {}", worktree.path);
                    }
                } else {
                    println!("applied: false (dry-run)");
                }
            }
        }
...
fn handle_context_command(
    ...
    match command {
        ContextCommand::Set {
            ...
            // If this worktree has an active workstream pointer, persist the updated context
            // snapshot into the global workstream registry (best-effort).
